...
def get_answers_by_owner(db: Session, user_id: int):
    return db.query(Answer).options(joinedload(Answer.question).joinedload(Question.owner)).filter(Answer.owner_id == user_id).order_by(Answer.created_at.desc()).all()
